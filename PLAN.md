...
**Phase 5 — AI deeper reading** ✅ _done_
- `ai.rs`: streaming Claude call (`claude-opus-4-8`, adaptive thinking, SSE)
  via `reqwest::blocking` on a worker `std::thread` + `mpsc` channel polled by a
  Bevy system — simpler than the tokio bridge originally sketched, no async
  runtime in the ECS. `D` inside the full-reading overlay requests the deeper
  reading; deltas stream into the panel body live; a hint line shows
  availability / progress / errors. Enabled when a key is found in
  `ANTHROPIC_API_KEY` or the OS keyring; `TARO_AI_MODEL` overrides the model,
  `TARO_DEEP_READING_AT` fires one request for the harness. Redeals reset the
  stream. SSE parsing + prompt builder are unit-tested; the HTTP error path was
  verified against the live API (401 renders in the hint).

**Phase 6 — polish & packaging** _(in progress)_
- Done: theming (4 runtime themes), AppImage + release CI, push CI (clippy +
  tests), **Flatpak** (`packaging/flatpak/build-flatpak.sh` →
  `flatpak run io.github.aleqpeia.Taro`), **keyring** API-key storage
  (`taro-app --set-api-key`), **scrollable** full-reading overlay (bevy_ui;
  wheel / ↑↓ / PageUp/Dn / Home/End).
- Remaining: more spreads behind the `Spread` trait, sound.
